...
\subsection{Attractors and Convergence}
    ...
    In nonlinear dynamical systems, there may exist sets of states in the phase space which show an attracting behaviour. This means that once a trajectory reaches an element of such a set, all of its future states will also be part of that set. Define an attractor as a set of states:
...
\scalebox{1.35}{
    ...
    \node (setParam)[process, scale=.5, below of= system, xshift = -2cm]{Apply Parameters to System};
    ...
    \node (simSys)[process, scale=.5, below of=samplDist]{Simulate Disturbed System};
    \node (eval)[process, scale=.5, below of=simSys]{Evaluate Convergence};
    ...
    \draw[->]   (pBounds.south) -| ([xshift=-3cm]opt);
    ...
    \draw[->] (samplDist) -- node[scale=.4]{Disturbances\\$\{s_i\}, i=\{1,2,...,n_s\}$}(simSys);
